...
class LogParser:
    ...
    def parse(self, logName):
        ...
        for idx, line in self.df_log.iterrows():
            logID = line['LineId']
            #logmessageL = self.preprocess(line['Command']).strip().split()
            logmessageL = list(
                filter(lambda x: x != '', re.split(r'[\s=:,/]', self.preprocess(line['Command']))))
            # logmessageL = list(filter(lambda x: x != '', re.split('[\s=:,]', self.preprocess(line['Command']))))
    ...
    def log_to_dataframe(self, log_file, regex, headers, logformat):
        ...
        with open(log_file, 'r') as fin:
            for line in fin.readlines():
                line = re.sub(r'[^\x00-\x7F]+', '<NASCII>', line.lower())
                try:
    ...
    def get_parameter_list(self, row):
        ...
        parameter_list = re.findall(template_regex, row["Command"])
